...
namespace
{
    ...
    {
    public:
        HttpStatus getStatus() const override { return status_; }
        const std::string& getReason() const override { return reason_; }
    ...
    std::future<std::unique_ptr<siesta::client::Response>> doRequest(
        Method method,
        ...
        const int timeout_ms)
    {
        auto f = std::async(
            std::launch::async,
            [=]() -> std::unique_ptr<siesta::client::Response> {
                ...
                nng_aio_set_timeout(
                    aio, timeout_ms > 0 ? timeout_ms : NNG_DURATION_INFINITE);
                ...
                if ((rv = nng_http_req_set_method(
                         req, method_str[int(method)])) != 0) {
                ...
                if (!body.empty()) {
                    ...
                    if (!content_type.empty()) {
                        if ((rv = nng_http_req_add_header(
                                 req, "content-type", content_type.c_str())) !=
                            0) {
                ...
                std::unique_ptr<Response> retval(new ResponseImpl());
                ResponseImpl* r = (ResponseImpl*)retval.get();

                if (nng_http_res_get_status(res) != NNG_HTTP_STATUS_OK) {
                    r->status_ =
                        static_cast<HttpStatus>(nng_http_res_get_status(res));
                    r->reason_ = nng_http_res_get_reason(res);
                } else {
                    const char* hdr;
                    ...
                    // required.)
                    if ((hdr = nng_http_res_get_header(
...
std::future<std::unique_ptr<siesta::client::Response>>
siesta::client::getRequest(const std::string& address, const int timeout_ms)
{
    return doRequest(Method::GET, address, "", "", timeout_ms);
}

std::future<std::unique_ptr<siesta::client::Response>>
siesta::client::putRequest(const std::string& address,
                           const std::string& body,
                           const std::string& content_type,
                           const int timeout_ms)
{
...
std::future<std::unique_ptr<siesta::client::Response>>
siesta::client::postRequest(const std::string& address,
                            const std::string& body,
                            const std::string& content_type,
                            const int timeout_ms)
{
...
std::future<std::unique_ptr<siesta::client::Response>>
siesta::client::deleteRequest(const std::string& address, const int timeout_ms)
{
    return doRequest(Method::DELETE, address, "", "", timeout_ms);
}

std::future<std::unique_ptr<siesta::client::Response>>
siesta::client::patchRequest(const std::string& address,
                             const std::string& body,
                             const std::string& content_type,
                             const int timeout_ms)
{
